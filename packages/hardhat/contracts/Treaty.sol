...
contract Treaty is TreatyStateMachine, Lockable {
  ...
  function makeActive() public inState(States.Draft) stateChange() virtual {
  ...
  function signTreaty() public inState(States.Active) stateChange() virtual returns (bool) {
    require(
      signatureState[msg.sender] == SignatureState.Unsigned,
  ...
  function signHash(bytes32 _hash) public inState(States.Active) stateChange() virtual returns (bool) {
    require(
      signatureState[msg.sender] == SignatureState.Unsigned,
      "Unexpected signature"
    );
